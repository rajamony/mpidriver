/*-----------------------------------------------------------------------*/
/* Program: Stream                                                       */
/* Revision: $Id: orig_stream.c,v 1.1 2011/02/09 22:50:12 eleon Exp $ */
...
int
HPCC_Stream(int vectorsize, int doIO, double *copyGBs, double *scaleGBs, double *addGBs,
  double *triadGBs, int *failure) {
    ...
    a = (double *) calloc (VectorSize, sizeof(double));
    b = (double *) calloc (VectorSize, sizeof(double));
    c = (double *) calloc (VectorSize, sizeof(double));
...
main ()
{
    ...
    int failure;
    HPCC_Stream (100*1000000, 1, &copyGBs, &scaleGBs, &addGBs, &triadGBs, &failure);
}
#endif
